...
$queryblog = mysqli_query($conn,'select * from blog order by date desc');
...
<body>
  ...
  <section class="contact">
    <h1 class="titrinfo">اخبار</h1>
    <div class="row">
        <?php
        while($a= mysqli_fetch_array($queryblog,MYSQLI_ASSOC)){
            echo '<div class="col-3 mt-3">';
                echo '<div class="card">';
                echo '<img class="card-img-top" src="'.$a['imageshakhes'].'">';
                echo '<div class="card-body">';
                    echo '<h5 class="card-title">'.$a['title'].'</h5>';
                echo ' <a href="postblog.php?id='.$a['blogid'].'" class="btn btn-primary mt-3">ادامه</a>';
                echo '</div>';
                echo '</div>';
            echo '</div>';
        }
