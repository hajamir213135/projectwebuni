...
$querytasviri = mysqli_query($conn,'select * from gozareshtasviri order by date desc');
...
<body>
  ...
  <section class="contact">
    <h1 class="titrinfo">گزارش تصویری</h1>
    <div class="row">
        <?php
        while($tasviri= mysqli_fetch_array($querytasviri,MYSQLI_ASSOC)){
            echo '<div class="col-3 mt-3">';
                echo '<div class="card">';
                echo '<img class="card-img-top" src="'.$tasviri['image1'].'">';
                echo '<div class="card-body">';
                    echo '<h5 class="card-title">'.$tasviri['title'].'</h5>';
                echo ' <a href="posttasviri.php?id='.$tasviri['gtid'].'" class="btn btn-primary mt-3">ادامه</a>';
                echo '</div>';
                echo '</div>';
            echo '</div>';
        }
